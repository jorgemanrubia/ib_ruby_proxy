...
The IB API define a number of [value object](https://martinfowler.com/bliki/ValueObject.html) classes to represent the interchanged data. For example, `com.ib.client.Bar` represents a candlestick of market data.

`ib_ruby_proxy` includes a code generation utility that analyzes the IB Java classes and generates:

* For the client side, a Ruby representation of each IB class. These classes contains all the data properties and, also, a method for converting them into their Java counterparts. 
* For the server side, a Ruby extension to the Java class to add a method to convert them into their Ruby counterparts. This is meant for internal use when invoking api methods.

The list of classes that are generated are contained in the property `classes` of `lib/ib_ruby_proxy/config.yml`. 
...
[ib-ruby](https://github.com/ib-ruby/ib-ruby) is a mature Ruby alternative for using Interactive Brokers that uses a different approach: it interacts with IB software by using lower level messages interchanges via sockets. It also offers a different and higher-level abstraction of the IB api. I personally prefer the approach of `ib_ruby_proxy` (I wouldn't have created it otherwise), but `ib-ruby` has been around for a long time, it is well maintained and eliminates the dependency of JRuby. You should definitely give it a try if you are thinking in invoking IB from Ruby.
